...
date_default_timezone_set('Europe/London'); //required if not set
$time = strtotime("-4 year", time());
$date = date("Y-m-d", $time);

// getting total number records without any search
$sql = "select a.assetID as ID, a.AssetNumber as ASSETNO, c.AssetCategory as ASSET, o.Office as OFFICE, a.DateIssued as DATE,e.EmployeeName as NAME,a.SerialNumber as SERIAL, a.AssetDescription as DESCR FROM assets as a INNER JOIN employees as e on e.employeeID = a.Employee INNER JOIN office as o on o.officeID = a.Office INNER JOIN asset_category as c on c.assetCategoryID = a.assetCategory WHERE a.DateIssued < '$date'";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = $row['DATE'];
	$nestedData[] = "<button class='btn btn-neutral btn-round btn-info' type='button'data-toggle='modal' data-target='#vendordetails' onclick='dets({$row['ID']})'><i class='fa fa-pencil'></i></button>";
