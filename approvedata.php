...
$sql = "select r.requestID as ID, a.AssetCategory as ASSET, d.DivisionName as DIVISION, r.DateRequested as DATE,e.EmployeeName as NAME FROM requests as r INNER JOIN employees as e on e.employeeID = r.Employee INNER JOIN divisions as d on d.divisionID = r.Division INNER JOIN asset_category as a on a.assetCategoryID = r.Asset where r.status='Approved'";
...
$query=mysqli_query($conn, $sql) or die("approvedata.php: get information");
